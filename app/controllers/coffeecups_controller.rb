  ...
  def show
    if lang_ok?
      if current_user.study_enabled == true
        if time_ok?
          Viewlist.create(:lang => I18n.locale, :page_type => "coffeecups", :page_id => params[:page], :user_id => current_user.id)
      ...
      Viewlist.create(:lang => I18n.locale, :page_type => "coffeecups", :page_id => params[:page], :user_id => 0)
      render template: "coffeecups/#{params[:page]}"
    end
  end
